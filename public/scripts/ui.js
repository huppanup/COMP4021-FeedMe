...
const ChatPanel = (function() {
    ...
    const updateStatus = function(user){
        clearTimeout(timeout);
        if (!(Authentication.getUser().username == user.username)) $("#chat-status").html(user.name + " is currently typing...");
        timeout = setTimeout(() => $("#chat-status").html(""), 3000);
    }

    return { initialize, update, addMessage, updateStatus };
})();
